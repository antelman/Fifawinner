...
function parsePastedOdds(text) {
  // ניקוי מספרי קו השערים ("מעל 2.5") — הם תיאור השוק, לא יחס
  const clean = text.replace(/(מעל|מתחת|over|under|קו|line)\s*[0-6][.,]5/gi, "$1");
  const matches = (clean.match(/\d+[.,]\d{1,2}/g) || [])
